...
//           #{order.id} · {fmtDate(order.createdAt ||order.created_at)}
//         </span>
...
//           {/* Payment badge */}
//           <span className={`text-xs px-2 py-1 rounded-full font-medium ${order.payment_status === 'PAID' ? 'bg-green-100 text-green-600'
...
//                   className={`flex flex-col items-center gap-1.5 px-4 py-3.5 flex-1 min-w-[80px] transition-all relative ${isActive ? `${t.color} ${t.bg}` : 'text-gray-400 hover:bg-gray-50'
//                     }`}
//                 >
//                   {isActive && (
...
//   );
// }
// frontend/src/app/pages/MyOrdersPage.tsx
import React, { useCallback, useEffect, useState } from 'react';
import { useNavigate, useSearchParams } from 'react-router';
import { useAuth } from '../contexts/AuthContext';
import StoreHeader from '../components/StoreHeader';
...
const TABS = [
  ...
  },
  {
    ...
    color: 'text-green-500', bg: 'bg-green-50',
    match: (o: Order) =>
      o.order_status === 'DELIVERED' && o.payment_status === 'PAID' && !o.is_reviewed,
  },
  {
    key: 'cancelled',
    label: 'Đã hủy',
    ...
    color: 'text-red-400', bg: 'bg-red-50',
    match: (o: Order) => o.order_status === 'CANCELLED' || o.order_status === 'REFUNDED',
  },
  {
    key: 'reviewed',
    label: 'Đánh giá',
    ...
    color: 'text-amber-400', bg: 'bg-amber-50',
    match: (o: Order) =>
      o.order_status === 'DELIVERED' && o.payment_status === 'PAID' && o.is_reviewed === true,
  },
];
...
function ReviewModal({ order, onClose, onSubmit }: {
  ...
  const handleSubmit = async () => {
    for (const item of order.items) {
      if (!ratings[item.product_id]) { alert('Vui lòng chấm sao cho tất cả sản phẩm!'); return; }
    }
    setSubmitting(true);
    try {
      await onSubmit(order.id, order.items.map(item => ({
        product_id: item.product_id, rating: ratings[item.product_id] ?? 5, comment: comments[item.product_id] ?? '',
      })));
      onClose();
          ...
          {order.items.map(item => (
            <div key={item.id} className="border border-gray-100 rounded-xl p-4 space-y-3">
              <div className="flex items-center gap-3">
                ...
                <p className="text-sm font-semibold text-gray-800 flex-1 leading-snug">{item.product?.name ?? 'Sản phẩm'}</p>
              </div>
              <div>
                <p className="text-xs text-gray-500 mb-1.5 font-medium">Chất lượng sản phẩm</p>
                <StarRating value={ratings[item.product_id] ?? 0} onChange={v => setRatings(prev => ({ ...prev, [item.product_id]: v }))} />
              </div>
              <textarea rows={2} value={comments[item.product_id] ?? ''}
                onChange={e => setComments(prev => ({ ...prev, [item.product_id]: e.target.value }))}
                placeholder="Chia sẻ trải nghiệm của bạn về sản phẩm này…"
                className="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm text-gray-700 resize-none focus:outline-none focus:border-cyan-400 transition-colors"
              />
...
function OrderCard({ order, onReview }: { order: Order; onReview: (o: Order) => void }) {
  ...
  const canReview = order.order_status === 'DELIVERED' && order.payment_status === 'PAID' && !order.is_reviewed;
  const isReviewed = order.is_reviewed === true;
          ...
          return (
            <div key={item.id} className="flex items-center gap-4 px-5 py-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                {(item.size || item.color) && (
                  <div className="flex items-center gap-1 mt-0.5 flex-wrap">
                    {item.size && <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded bg-blue-50 text-blue-600 border border-blue-100">Size: {item.size}</span>}
                    {item.color && <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded bg-pink-50 text-pink-600 border border-pink-100">Màu: {item.color}</span>}
                  </div>
                )}
                <p className="text-xs text-gray-400 mt-0.5">x{item.quantity}</p>
              </div>
      ...
      <div className="flex items-center justify-between px-5 py-4 border-t border-gray-50">
        ...
        <div className="flex items-center gap-2 flex-wrap justify-end" onClick={e => e.stopPropagation()}>
          ...
          <span className={`text-xs px-2 py-1 rounded-full font-medium ${
            ...
            : order.payment_status === 'REFUNDED' ? 'bg-purple-100 text-purple-600'
            : order.payment_status === 'FAILED' ? 'bg-red-100 text-red-500'
            : 'bg-yellow-100 text-yellow-600'
          }`}>
            {order.payment_status === 'PAID' ? '✓ Đã thanh toán'
          ...
          {/* Refunded indicator */}
          {isRefunded && (
            <span className="text-xs px-2 py-1 rounded-full bg-purple-50 text-purple-600 font-medium border border-purple-100">
              💜 Tiền đã hoàn vào ví
            </span>
          )}
...
export default function MyOrdersPage() {
  ...
  const handleReviewSubmit = async (orderId: string, items: { product_id: string; rating: number; comment: string }[]) => {
    for (const item of items) {
      await fetch(`${API_BASE_URL}/reviews`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
        body: JSON.stringify({ order_id: orderId, product_id: item.product_id, rating: item.rating, comment: item.comment }),
      });
    }
    await fetchOrders();
    setSearchParams({ tab: 'reviewed' });
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="mx-auto w-full max-w-screen-2xl px-4 py-6">
            ...
            {TABS.map(t => {
              ...
              return (
                <button key={t.key} onClick={() => setSearchParams({ tab: t.key })}
                  className={`flex flex-col items-center gap-1.5 px-4 py-3.5 flex-1 min-w-[80px] transition-all relative ${isActive ? `${t.color} ${t.bg}` : 'text-gray-400 hover:bg-gray-50'}`}>
                  {isActive && <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-current rounded-t-full" />}
                  <div className="relative">
                    {t.icon}
                    {counts[t.key] > 0 && (
                      <span className="absolute -top-1.5 -right-1.5 min-w-[16px] h-4 px-0.5 rounded-full bg-red-500 text-white text-[9px] font-extrabold flex items-center justify-center">
                  ...
                  <span className={`text-[10px] font-semibold leading-tight text-center ${isActive ? '' : 'text-gray-400'}`}>{t.label}</span>
                </button>
              );
            })}
          </div>
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center h-56 gap-4 bg-white rounded-2xl border border-gray-100">
            <div className={`w-16 h-16 rounded-2xl ${tab.bg} ${tab.color} flex items-center justify-center`}>{tab.icon}</div>
            <div className="text-center">
              <p className="font-semibold text-gray-700">Chưa có đơn hàng</p>
              <p className="text-sm text-gray-400 mt-1">Không có đơn nào ở trạng thái "{tab.label}"</p>
            </div>

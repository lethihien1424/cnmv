...
const scanBusinessLicense = async (req, res) => {
  try {
    ...
    const store = await Store.findByPk(storeId, {
      attributes: ["id", "store_name", "tax_code", "business_license", "status"],
    });
    ...
    // Bước 2: Lấy tên file ảnh từ DB
    // ⚠️  Tên cột trong DB của dự án này là: business_license
    const imageName = extractFilename(store.business_license);

    if (!imageName) {
      return res.status(400).json({
        message:
          "Cửa hàng này chưa có dữ liệu ảnh GPKD trong Database. " +
    ...
    // Bước 5: KÍCH HOẠT AI TESSERACT.JS
    console.log("🤖 AI đang bắt đầu đọc ảnh:", imagePath);

    const ocrResult = await Tesseract.recognize(
      imagePath,
      "vie+eng", // Đọc cả tiếng Việt lẫn tiếng Anh (GPKD thường có cả hai)
      {
        logger: (m) => {
          // In tiến trình ra terminal để dễ theo dõi
          if (m.status === "recognizing text") {
            process.stdout.write(
              `\r🔍 Đang nhận dạng: ${Math.round((m.progress || 0) * 100)}%`
            );
          }
        },
      }
    ...
    if (!rawText.trim() || rawText.trim().length < 5) {
      return res.status(422).json({
        message:
          "AI không đọc được nội dung từ ảnh. Vui lòng dùng ảnh rõ nét, chụp thẳng góc.",
        rawText,
      });
    }
